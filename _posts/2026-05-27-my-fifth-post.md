...
In a multi-tenancy enterprise data system, each tenant has their own DB and data. A data streaming service is foundational infrastructure that enables the building of other microservices and apps that consume the data, without placing additional load on the production DB. The assumption is that the system must be always on and stream data in near real time (rather than in batches). Therefore, Kafka is the default technology chosen to provide this capability. The system ensures that change is captured, delivered and available for other services to react to. This service should decouples producers from consumers. The application writes to DB normally, and everyone else subscribe to Kafka topics.
